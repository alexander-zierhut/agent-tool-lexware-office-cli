...
from lexwareoffice import __version__
from lexwareoffice.cli import app


def test_report_runs_offline_and_names_this_repo(monkeypatch, tmp_path):
    monkeypatch.setenv("LEXWAREOFFICE_CONFIG_DIR", str(tmp_path / "does-not-exist"))
    monkeypatch.delenv("LEXWAREOFFICE_TOKEN", raising=False)

    result = CliRunner().invoke(app, ["report"])
    assert result.exit_code == 0, result.output

    data = json.loads(result.stdout)
    assert data["repo"] == "alexander-zierhut/agent-tool-lexware-office-cli"
    assert data["published"] is True
    assert data["version"] == __version__
    assert data["issues"].endswith("/agent-tool-lexware-office-cli/issues")
